...
DESCRIPTION: Computes exact transit durations using DATE_DIFF and measures 
             the operational "Logistics Drag" of distressed markets both in 
             absolute days and relative percentage variance against the global baseline.
===============================================================================
*/
...
WITH country_logistics AS (
  SELECT
    ...
    ROUND(AVG(DATE_DIFF(ship_date, order_date, DAY)), 2) AS avg_country_lead_time_days
...
global_logistics_baseline AS (
  SELECT
    ROUND(AVG(DATE_DIFF(ship_date, order_date, DAY)), 2) AS global_avg_lead_time_days
  FROM `global-supply-project.gym_supply_portfolio.vw_superstore_clean`
)
...
SELECT
  ...
  -- 1. Absolute deviation in transit days
